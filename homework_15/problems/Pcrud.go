...
func (p *ProblemRepo) Create(problem Problem) error {
	...
	problem.Descriptions, problem.Level)
	if err != nil{
		fmt.Println(err)
	}
	return err
...
func (p *ProblemRepo) Get() ([]Problem, error){
	...
	for rows.Next(){
		problem := Problem{}
		err = rows.Scan(&problem.Id, &problem.Descriptions, &problem.Level)
		if err != nil{
			return nil, err
		}
